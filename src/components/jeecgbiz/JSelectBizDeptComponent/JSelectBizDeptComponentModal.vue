    ...
    <a-row :gutter="18">
      ...
      <a-col :span="12">
        ...
        <a-form layout="inline" class="j-inline-form">
          ...
          <!-- 动态生成的查询条件 -->
          <j-select-biz-dept-query-item v-if="queryConfig.length>0" v-show="showMoreQueryItems" :queryParam="queryParam" :queryConfig="queryConfig" @pressEnter="searchQuery"/>
          <!-- 按钮 -->
...
<script>
  ...
  import JSelectBizDeptQueryItem from './JSelectBizDeptQueryItem'
  import { queryDepartTreeList } from '@/api/api'

  export default {
    name: 'JSelectBizDeptComponentModal',
    mixins: [JeecgListMixin],
    components: {Ellipsis, JSelectBizDeptQueryItem},
    props: {
      value: {
        type: Array,
        default: () => []
    ...
    methods: {
      ...
      initQueryUserByDepId(selectedDepIds) {
        this.queryParam.departIds = selectedDepIds[0];
        this.loading = true
        getAction(this.valueUrl || this.listUrl, {
          // 这里最后加一个 , 的原因是因为无论如何都要使用 in 查询，防止后台进行了模糊匹配，导致查询结果不准确
          // [this.valueKey]: value.join(',') + ',',
