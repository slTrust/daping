...
export function ChartFont(res) {
    ...
    let fontSize = 150 * (clientWidth / 1920 / 6); // 1920为设计稿尺寸，100为1rem=100px的换算比，可根据实际情况调整
    return res * fontSize;
}
/**
 * html根样式字体适配
 * @unit 针对大屏，一个大屏的宽度是几个小屏
 * */
export function setRootFontSize(unit) {
  // debugger
  //设计稿尺寸
  let psdWidth = 1920;
  //预设基准值
  let baseFontSize = 32;
  let width = window.screen.availWidth || window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
  // let height = window.screen.availHeight;
  let rootHtml = document.documentElement;
  if(!unit) {
    unit = 6;//因暂时是有大屏用到，所以写死为6
  }
  let rem = baseFontSize / psdWidth * width /unit;
