<template>
  <a-card :bordered="false">
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="audit(record)" v-if="record.checkStatus == 0 || record.checkStatus == 4">审核</a>

          <a-divider type="vertical" v-if="record.checkStatus == 0 || record.checkStatus == 4" />
    ...
    <to-audit-modal ref="modalForm" @ok="modalFormOk"></to-audit-modal>
  </a-card>
</template>
...
import ToAuditModal from './ToAuditModal'
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/enterprise/enterpriseWorkOrderCheck/todoPageList',
  ...
  methods: {
    ...
    audit(record) {
      this.$refs.modalForm.showAuditLog(record)
    ...
    auditLogFrom: function (record) {
      ...
      this.$refs.auditLog.title = '企业迁出/注销审核日志'
